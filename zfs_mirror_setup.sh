...
#
# ============================================================================
# WARNING: SINGLE-OS SYSTEMS ONLY
# ============================================================================
# This script is designed EXCLUSIVELY for dedicated Ubuntu server installations
# where Ubuntu is the ONLY operating system on the machine.
#
# NOT SUITABLE FOR:
...
#   - Desktop systems where you plan to install additional OSes
#
# WHY SINGLE-OS ONLY:
#   - Automatic boot entry management creates Ubuntu-specific UEFI entries
#   - Boot entry cleanup removes unexpected entries (may delete other OSes)
#   - Three-entry boot menu assumes full control of boot order
#   - Shutdown hooks modify UEFI NVRAM on every reboot
#
# If you need dual-boot or multi-OS support, DO NOT USE THIS SCRIPT.
# ============================================================================
#
# ============================================================================
...
case "$ARCH" in
    x86_64)
        readonly EFI_BOOTLOADER="shimx64.efi"
        readonly GRUB_PKG="grub-efi-amd64"
        readonly GRUB_PKG_SIGNED="grub-efi-amd64-signed"
        readonly GRUB_TARGET="x86_64-efi"
        ;;
    aarch64)
        readonly EFI_BOOTLOADER="shimaa64.efi"
        readonly GRUB_PKG="grub-efi-arm64"
        readonly GRUB_PKG_SIGNED="grub-efi-arm64-signed"
        readonly GRUB_TARGET="arm64-efi"
        ;;
    *)
        echo "ERROR: Unsupported architecture: $ARCH" >&2
        echo "" >&2
        echo "This script supports only:" >&2
        echo "  - x86_64 (Intel/AMD 64-bit)" >&2
        echo "  - aarch64 (ARM 64-bit)" >&2
        echo "" >&2
        echo "Your system architecture ($ARCH) is not supported." >&2
...
# sync-efi-partitions removed in v6.9.0 - redundant with grub-install to all drives
...
# Detect architecture and set package names
ARCH=$(uname -m)
case "$ARCH" in
    x86_64)
        GRUB_PKG="grub-efi-amd64"
        GRUB_PKG_SIGNED="grub-efi-amd64-signed"
        GRUB_TARGET="x86_64-efi"
        ;;
    aarch64)
        GRUB_PKG="grub-efi-arm64"
        GRUB_PKG_SIGNED="grub-efi-arm64-signed"
...
grub-install --target=$GRUB_TARGET --efi-directory=/boot/efi2 \
...
# Set up grub-install protection to prevent NVRAM modifications
log_info "Setting up grub-install protection wrapper..."
chroot /mnt dpkg-divert --divert /usr/sbin/grub-install.real --rename /usr/sbin/grub-install

cat > /mnt/usr/sbin/grub-install << 'GRUB_WRAPPER'
#!/bin/bash
# Wrapper to always use --no-nvram to prevent UEFI NVRAM modifications
# This ensures our carefully managed boot entries are not corrupted by
# automatic grub-install runs during package updates or manual operations.
#
# GENERATED BY: Ubuntu ZFS Mirror Root Installation Script v${VERSION}
# The real grub-install is at /usr/sbin/grub-install.real

exec /usr/sbin/grub-install.real --no-nvram "$@"
GRUB_WRAPPER

chmod +x /mnt/usr/sbin/grub-install
log_info "✓ grub-install wrapper installed (all calls will use --no-nvram)"
...
# ==============================================================================
# DRIVE DISCOVERY: zpool is the source of truth for drive health
# ==============================================================================
# We use a two-stage discovery process to ensure we only sync to healthy drives:
#
# 1. Query zpool status rpool to find ONLINE drives (the authoritative source)
#    - Only drives showing ONLINE status are candidates for sync
#    - This ensures we skip DEGRADED, UNAVAIL, FAULTED, or OFFLINE drives
#    - Prevents mount failures when trying to access failed drives
#
# 2. Query blkid to find all EFI partitions by UUID
#    - Both mirror drives have EFI partitions with identical UUIDs
#    - We need blkid to find all physical EFI partition paths
#
# 3. Filter: Only include EFI partitions on drives that are ONLINE in zpool
#    - Cross-reference blkid results against zpool ONLINE drives list
#    - This is the key to resilient drive failure handling
#
# This design ensures the sync script gracefully handles drive failures without
# errors or confusion, maintaining the mirror's redundancy even in degraded state.
# ==============================================================================

# Discover ONLINE drives from rpool (source of truth for healthy drives)
log_info "Discovering ONLINE drives from rpool..."
RPOOL_DRIVES=()
if ! zpool status rpool &>/dev/null; then
    log_error "Cannot access rpool - is ZFS loaded?"
    exit 1
fi
...
for efi_part in "\${EFI_PARTITIONS[@]}"; do
    ...
    if [[ "\$efi_part" == "\$MOUNTED_EFI" ]]; then
        ...
        if grub-install --target=$GRUB_TARGET --efi-directory=/boot/efi --bootloader-id="Ubuntu-\${DRIVE_ID}" --recheck --no-floppy "\$base_drive" >/dev/null 2>&1; then
        ...
        if mount "\$efi_part" "\$TEMP_MOUNT"; then
            ...
            if grub-install --target=$GRUB_TARGET --efi-directory="\$TEMP_MOUNT" --bootloader-id="Ubuntu-\${DRIVE_ID}" --recheck --no-floppy "\$base_drive" >/dev/null 2>&1; then
...
#!/bin/bash
#
# Unified ZFS Mirror Boot Synchronization Script (v6.10.2)
#
# GENERATED BY: Ubuntu ZFS Mirror Root Installation Script v${VERSION}
# SOURCE: ${ORIGINAL_REPO}
# PURPOSE: Syncs GRUB to all mirror drives and rotates boot entry on shutdown
#
# USAGE:
#   sync-mirror-boot           # Normal mode: sync GRUB only (kernel updates)
#   sync-mirror-boot --shutdown # Shutdown mode: sync GRUB + rotate boot entry
...
# Detect architecture and set EFI bootloader filename
ARCH=$(uname -m)
case "$ARCH" in
    x86_64)
        EFI_BOOTLOADER="shimx64.efi"
        ;;
    aarch64)
        EFI_BOOTLOADER="shimaa64.efi"
        ;;
    *)
...
if [[ "${1:-}" == "--shutdown" ]]; then
    SHUTDOWN_MODE=true
    log_info "=== ZFS Mirror Boot Sync (Shutdown Mode) ==="
else
    log_info "=== ZFS Mirror Boot Sync (Normal Mode) ==="
fi
...
# Get drive labels by mounting EFI partitions and checking /EFI/ directories
LABEL1=""
LABEL2=""
for idx in 0 1; do
    efi_part="${RPOOL_DRIVES[$idx]}-part1"
    temp_mount=$(mktemp -d)
...
while IFS= read -r line; do
    ...
    elif [[ "$line" =~ ^Boot([0-9A-F]{4})\*[[:space:]]Ubuntu[[:space:]]-[[:space:]]${LABEL1}[[:space:]] ]]; then
        boot_num="${BASH_REMATCH[1]}"
        if [[ "$line" =~ HD\( ]]; then
    ...
    elif [[ "$line" =~ ^Boot([0-9A-F]{4})\*[[:space:]]Ubuntu[[:space:]]-[[:space:]]${LABEL2}[[:space:]] ]]; then
        boot_num="${BASH_REMATCH[1]}"
        if [[ "$line" =~ HD\( ]]; then
...
if [[ -z "$ROTATING_ENTRY" ]]; then
    ...
    if efibootmgr --create --gpt \
        --disk "${RPOOL_DRIVES[0]}" \
        --part 1 \
        --loader "\\EFI\\${LABEL1}\\$EFI_BOOTLOADER" \
...
if [[ -z "$STATIC1_ENTRY" ]]; then
    ...
    if efibootmgr --create --gpt \
        ...
        >/dev/null 2>&1; then
        while IFS= read -r line; do
            if [[ "$line" =~ ^Boot([0-9A-F]{4})\\*[[:space:]]Ubuntu[[:space:]]-[[:space:]]${LABEL1}[[:space:]] ]]; then
...
if [[ -z "$STATIC2_ENTRY" ]]; then
    ...
    if efibootmgr --create --gpt \
        --disk "${RPOOL_DRIVES[1]}" \
        --part 1 \
        --loader "\\EFI\\${LABEL2}\\$EFI_BOOTLOADER" \
        --label "Ubuntu - $LABEL2" \
        >/dev/null 2>&1; then
        while IFS= read -r line; do
            if [[ "$line" =~ ^Boot([0-9A-F]{4})\\*[[:space:]]Ubuntu[[:space:]]-[[:space:]]${LABEL2}[[:space:]] ]]; then
...
if [[ "$SHUTDOWN_MODE" == "true" ]]; then
    ...
    if [[ ${#RPOOL_DRIVES[@]} -ne 2 ]]; then
        log_warning "Expected 2 drives in rpool, found ${#RPOOL_DRIVES[@]}, skipping rotation"
        exit 0
    fi

    # Get PARTUUIDs for both drives (part1 = EFI partition)
    PARTUUID1=$(blkid -s PARTUUID -o value "${RPOOL_DRIVES[0]}-part1" 2>/dev/null || echo "")
    PARTUUID2=$(blkid -s PARTUUID -o value "${RPOOL_DRIVES[1]}-part1" 2>/dev/null || echo "")

    if [[ -z "$PARTUUID1" ]] || [[ -z "$PARTUUID2" ]]; then
        log_error "Could not determine drive PARTUUIDs, skipping rotation"
        exit 0
    fi

    # Find current rotating entry
    ROTATING_ENTRY=""
    ROTATING_PARTUUID=""
    while IFS= read -r line; do
        if [[ "$line" =~ ^Boot([0-9A-F]{4}).*Ubuntu[[:space:]]-[[:space:]]Rotating ]]; then
            ROTATING_ENTRY="${BASH_REMATCH[1]}"
            # Extract PARTUUID from HD(...) portion
            if [[ "$line" =~ HD\(1,GPT,([a-f0-9-]+), ]]; then
                ROTATING_PARTUUID="${BASH_REMATCH[1]}"
            fi
            break
        fi
    done < <(efibootmgr 2>/dev/null)
    ...
    if [[ -z "$ROTATING_ENTRY" ]]; then
        log_warning "No rotating boot entry found, skipping rotation"
        exit 0
    fi

    # Determine which drive to rotate to
    if [[ "$ROTATING_PARTUUID" == "$PARTUUID1" ]]; then
        # Currently pointing to drive 1, rotate to drive 2
        NEXT_PARTUUID="$PARTUUID2"
        NEXT_DRIVE="${RPOOL_DRIVES[1]}"
        log_info "Rotating from drive 1 to drive 2"
    else
        # Currently pointing to drive 2 or unknown, rotate to drive 1
        NEXT_PARTUUID="$PARTUUID1"
        NEXT_DRIVE="${RPOOL_DRIVES[0]}"
        log_info "Rotating from drive 2 to drive 1"
    fi

    # Get drive label by mounting EFI partition and checking /EFI/ directory
    NEXT_EFI_PART="${NEXT_DRIVE}-part1"
    TEMP_MOUNT=$(mktemp -d)
    if ! mount "$NEXT_EFI_PART" "$TEMP_MOUNT" 2>/dev/null; then
        log_error "Could not mount $NEXT_EFI_PART"
        rmdir "$TEMP_MOUNT"
        exit 1
    fi

    NEXT_LABEL=$(find "$TEMP_MOUNT/EFI/" -maxdepth 1 -name "Ubuntu-*" -type d 2>/dev/null | head -1 | xargs -r basename)
    umount "$TEMP_MOUNT"
    rmdir "$TEMP_MOUNT"

    if [[ -z "$NEXT_LABEL" ]]; then
        log_error "Could not determine drive label for next boot entry"
        exit 1
    fi

    # Get current boot order before making changes
    CURRENT_ORDER=$(efibootmgr | grep "^BootOrder:" | cut -d: -f2 | tr -d ' ')

    # Remove old rotating entry
    log_info "Removing old rotating entry Boot$ROTATING_ENTRY"
    if ! efibootmgr -b "$ROTATING_ENTRY" -B >/dev/null 2>&1; then
        log_error "Failed to remove old rotating entry"
        exit 1
    fi

    # Create new rotating entry
    log_info "Creating new rotating entry: Ubuntu - Rotating ($NEXT_LABEL)"
    if ! efibootmgr --create --gpt \
        --disk "$NEXT_DRIVE" \
        --part 1 \
        --loader "\\EFI\\${NEXT_LABEL}\\$EFI_BOOTLOADER" \
        --label "Ubuntu - Rotating ($NEXT_LABEL)" \
        >/dev/null 2>&1; then
        log_error "Failed to create new rotating entry"
        exit 1
    fi

    # Find new rotating entry number
    NEW_ROTATING_ENTRY=""
    while IFS= read -r line; do
        if [[ "$line" =~ ^Boot([0-9A-F]{4}).*Ubuntu[[:space:]]-[[:space:]]Rotating ]]; then
            NEW_ROTATING_ENTRY="${BASH_REMATCH[1]}"
            break
        fi
    done < <(efibootmgr 2>/dev/null)

    if [[ -z "$NEW_ROTATING_ENTRY" ]]; then
        log_error "Could not find newly created rotating entry"
        exit 1
    fi

    # Discover labels for both drives to identify expected static entries
    log_info "Discovering drive labels for cleanup..."
    DRIVE_LABELS=()
    for idx in 0 1; do
        efi_part="${RPOOL_DRIVES[$idx]}-part1"
        temp_mount=$(mktemp -d)
    ...
    # Find expected static entries (matching discovered drive labels)
    EXPECTED_STATIC_ENTRIES=()
    for label in "${DRIVE_LABELS[@]}"; do
        while IFS= read -r line; do
            if [[ "$line" =~ ^Boot([0-9A-F]{4})\*[[:space:]]Ubuntu[[:space:]]-[[:space:]]${label}[[:space:]] ]]; then
                EXPECTED_STATIC_ENTRIES+=("${BASH_REMATCH[1]}")
                break
            fi
        done < <(efibootmgr 2>/dev/null)
    done

    # Clean up old/unexpected Ubuntu entries
    log_info "Cleaning up old Ubuntu boot entries..."
    CLEANUP_COUNT=0
    while IFS= read -r line; do
        if [[ "$line" =~ ^Boot([0-9A-F]{4}).*Ubuntu ]]; then
            boot_num="${BASH_REMATCH[1]}"

            # Skip if it's the new rotating entry
            if [[ "$boot_num" == "$NEW_ROTATING_ENTRY" ]]; then
                continue
            fi

            # Skip if it's one of the expected static entries
            is_expected=false
            for expected in "${EXPECTED_STATIC_ENTRIES[@]}"; do
                if [[ "$boot_num" == "$expected" ]]; then
                    is_expected=true
                    break
                fi
            done
    ...
    if [[ $CLEANUP_COUNT -gt 0 ]]; then
        log_info "  Cleaned up $CLEANUP_COUNT old boot entry/entries"
    fi

    # Build new boot order: rotating first, then static entries, then others
    NEW_ORDER="$NEW_ROTATING_ENTRY"
    for entry in "${EXPECTED_STATIC_ENTRIES[@]}"; do
        NEW_ORDER="$NEW_ORDER,$entry"
    done

    # Add non-Ubuntu entries from current order
    IFS=',' read -ra ORDER_ARRAY <<< "$CURRENT_ORDER"
    for boot_num in "${ORDER_ARRAY[@]}"; do
        # Skip if it's a Ubuntu entry or the old rotating entry
        if [[ "$boot_num" == "$NEW_ROTATING_ENTRY" ]] || \
           [[ "$boot_num" == "$ROTATING_ENTRY" ]] || \
           [[ " ${EXPECTED_STATIC_ENTRIES[*]} " =~ " ${boot_num} " ]]; then
            continue
        fi
        NEW_ORDER="$NEW_ORDER,$boot_num"
    done
    ...
    # Set new boot order
    log_info "Setting boot order: $NEW_ORDER"
    if ! efibootmgr -o "$NEW_ORDER" >/dev/null 2>&1; then
        log_error "Failed to set boot order"
        exit 1
    fi

    log_info "✓ Boot entry rotated to $NEXT_LABEL for next boot"
fi
...
[Unit]
Description=ZFS Mirror Boot Sync on Shutdown - Sync GRUB and rotate boot entry
...
[Service]
Type=oneshot
ExecStart=/usr/local/bin/sync-mirror-boot --shutdown
RemainAfterExit=yes
TimeoutStartSec=60
...
# Sync bootloader files to all drives
log_info "Syncing GRUB to all drives including replacement"
echo "Syncing GRUB to all drives..."
/usr/local/bin/sync-mirror-boot
...
grub-install --target=$GRUB_TARGET --efi-directory=/boot/efi "\$REPLACEMENT_DRIVE"
...
Sync boot mirror:        sudo /usr/local/bin/sync-mirror-boot
...
# Sync bootloader to all drives to ensure changes are applied
log_info "Syncing boot mirror after cleanup..."
if [[ -x /usr/local/bin/sync-mirror-boot ]]; then
    if /usr/local/bin/sync-mirror-boot >/dev/null 2>&1; then
        log_info "✓ Boot mirror sync completed successfully"
    else
        log_warning "Boot mirror sync failed - drives may have inconsistent content"
    fi
else
    log_warning "Boot mirror sync script not found - partitions may be out of sync"
fi
...
if [[ ! "${response}" =~ ^[Nn]$ ]]; then
    ...
    echo ""
    echo -e "${GREEN}4. Verify boot mirror sync is working:${NC}"
    echo -e "   ${GREEN}sudo /usr/local/bin/sync-mirror-boot${NC}"
    echo ""
    ...
    echo -e "  • ${GREEN}/usr/local/bin/sync-mirror-boot${NC} - Sync bootloader to all drives + rotate boot entry"
