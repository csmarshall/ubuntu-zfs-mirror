...
#!/bin/bash
#
# Unified ZFS Mirror Boot Synchronization Script (v6.10.0)
#
# GENERATED BY: Ubuntu ZFS Mirror Root Installation Script v${VERSION}
# SOURCE: ${ORIGINAL_REPO}
...
if [[ "$SHUTDOWN_MODE" == "true" ]]; then
    ...
    # Get drive label (extract from EFI folder)
    EFI_UUID=$(awk '/\/boot\/efi/ && /^UUID=/ {gsub(/UUID=/, "", $1); print $1}' /etc/fstab 2>/dev/null || echo "")
    NEXT_EFI_PART="$(blkid --output device --match-token UUID="$EFI_UUID" --match-token PARTUUID="$NEXT_PARTUUID" 2>/dev/null | head -1)"

    if [[ -z "$NEXT_EFI_PART" ]]; then
        log_error "Could not find EFI partition for next drive"
        exit 1
    fi

    # Mount to discover folder name
    TEMP_MOUNT=$(mktemp -d)
    ...
    DRIVE_LABELS=()
    for partuuid in "$PARTUUID1" "$PARTUUID2"; do
        efi_part="$(blkid --output device --match-token UUID="$EFI_UUID" --match-token PARTUUID="$partuuid" 2>/dev/null | head -1)"
        if [[ -n "$efi_part" ]]; then
            temp_mount=$(mktemp -d)
            if mount "$efi_part" "$temp_mount" 2>/dev/null; then
    ...
    for label in "${DRIVE_LABELS[@]}"; do
        while IFS= read -r line; do
            if [[ "$line" =~ ^Boot([0-9A-F]{4}).*Ubuntu[[:space:]]-[[:space:]]${label}$ ]]; then
